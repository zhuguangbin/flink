...
import org.apache.flink.formats.avro.utils.AvroRecordConverter;
...
public class AvroRowSerializationSchema implements SerializationSchema<Row> {
	...
	public byte[] serialize(Row row) {
		try {
			// convert to record
			final GenericRecord record = AvroRecordConverter.convertRowToAvroRecord(schema, row);
			arrayOutputStream.reset();
			preRecordWrite(schema, arrayOutputStream);
	...
	/** Hook for sub-class to write before each record content. */
